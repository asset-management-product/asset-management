...
with finance:
    ...
    with income_statement_section:
        ...
        st.dataframe(dict_to_dataframe_preserve_order(income_data_dict[selected_ticker]))


    with balance_sheet_section:
        st.subheader("Balance Sheet - Bảng cân đối kế toán")
        # Your content for nested tab 2
        st.dataframe(dict_to_dataframe_preserve_order(bs_data_dict[selected_ticker]))
    with cash_flow_section:
        ...
        with direct_cash_flow_section:
            ...
            st.dataframe(dict_to_dataframe_preserve_order(direct_cash_flow_data_dict[selected_ticker]))
        with indirect_cash_flow_section:
            
            st.write(selected_ticker)
            st.dataframe(dict_to_dataframe_preserve_order(indirect_cash_flow_data_dict[selected_ticker]))
